...
def encode_apk(name):
    output = excuteCommand('cp -r '+pathDir+'/'+name+'/'+json_file+' ../app/src/main/assets')
    print "==拷贝新的json成功="
    if os.path.exists('../app/build/'):
        output = excuteCommand('rm -r ../app/build/')
    
    output = excuteCommand('cd ../ && ./gradlew assembleRelease')
