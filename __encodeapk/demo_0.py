...
def encode_apk(name):
    output = excuteCommand('cp -r apk/'+name+'/'+json_file+' ../src/image')
    print "==拷贝新的json成功="
    ...
    if os.path.exists('../android/app/build/outputs/apk/release/app-release.apk'):#
        output = excuteCommand('cp -r ../android/app/build/outputs/apk/release/app-release.apk apk/'+name)
